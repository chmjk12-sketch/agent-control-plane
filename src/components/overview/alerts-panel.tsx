  ...
  return (
    <Card className={alerts.length > 0 ? "border-red-500/20" : ""}>
      ...
      <CardContent className="space-y-3">
        {alerts.length === 0 ? (
          <p className="text-sm text-emerald-400 text-center py-4">All systems operational</p>
        ) : (
          alerts.map((a) => (
            <div key={a.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{a.status === "offline" ? "Agent offline" : "Performance degraded"}</p>
                </div>
              </div>
