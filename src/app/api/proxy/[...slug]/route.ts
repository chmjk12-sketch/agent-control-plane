...
// 路由格式: /api/proxy/{slug}/rest/of/path
async function proxyHandler(req: NextRequest, context: { params: Promise<{ slug: string[] }> }) {
  const { slug } = await context.params;
  // slug 是数组，第一个元素是 agent slug，其余是子路径
  const agentSlug = slug[0];
  const subPathSegments = slug.slice(1);
  ...
  const subPath = subPathSegments.length > 0 ? "/" + subPathSegments.join("/") : "/";
